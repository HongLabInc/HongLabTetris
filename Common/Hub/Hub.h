...
public:
	...
	explicit Hub(uint32_t id, Server& server, boost::asio::io_context& io, std::size_t roomSize);
	virtual ~Hub() = default;
	...
	void BroadcastChatMessage(uint32_t userID, std::string_view message) const;

	uint32_t GetID() const; //thread-safe
	Strand GetStrand() const; //thread-safe

	std::shared_ptr<User> GetUserOrNull(uint32_t userID); //thread-safe
	std::shared_ptr<Session> GetUserSessionOrNull(uint32_t userID); //thread-safe
